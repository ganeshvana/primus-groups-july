...
class ProductTemplate(models.Model):
	...
	@api.model
	def _default_secondary_uom_id(self):
	    uom = self.env['uom.uom'].search([('name', '=', 'Pcs')], limit=1)
	    if uom:
	        return uom[0]
	    return self.env['uom.uom']
		
	secondary_uom_id = fields.Many2one('uom.uom', string="Secondary UOM", default=_default_secondary_uom_id)
	secondary_uom_name = fields.Char(string='Sec. Unit of Measure Name', related='secondary_uom_id.name', readonly=True)
	secondary_qty = fields.Float('Secondary Qty', compute='_compute_second_quantities', search='_search_second_qty_available',
		digits = (12,3), compute_sudo=False)
